...
const ManageJob = () => {
    ...
    const fetchJobAndApplications = async () => {
        try {
            ...
            // Fetch job details - assuming we need to get it from myJobs
    ...
    const handleUpdateApplicationStatus = async (applicationId, status) => {
        try {
            ...
            setAlert({ ok: true, message: `Application ${status.toLowerCase()} successfully` });
            setTimeout(() => setAlert(null), 3000);
        } catch (err) {
            console.error("Error updating application status:", err);
            setAlert({ ok: false, message: err.response?.data?.message || "Failed to update application" });
            setTimeout(() => setAlert(null), 3000);
        } finally {
    ...
    const handleCloseJob = async () => {
        ...
        try {
            ...
            setAlert({ ok: false, message: err.response?.data?.message || "Failed to close job" });
            setTimeout(() => setAlert(null), 3000);
        } finally {
            setActionLoading(null);
        }
    ...
    const getStatusBadge = (status) => {
        const statusConfig = {
            PENDING: { variant: "default", className: "bg-yellow-100 text-yellow-800 hover:bg-yellow-100", icon: Clock },
            ACCEPTED: { variant: "default", className: "bg-green-100 text-green-800 hover:bg-green-100", icon: CheckCircle },
            REJECTED: { variant: "default", className: "bg-red-100 text-red-800 hover:bg-red-100", icon: XCircle },
        };
        ...
        <div className="min-h-screen bg-background py-8 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-muted-foreground mt-1">Review and manage applications for this position</p>
                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="flex items-center gap-3 text-muted-foreground">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-semibold text-foreground">{job?.salary || "Not specified"}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 text-muted-foreground">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-semibold text-foreground">{job?.address || "Not specified"}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 text-muted-foreground">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-semibold text-foreground">{applications.length}</p>
                                </div>
                            </div>
                        </div>
                        {job?.jobStatus === "OPEN" && (
...
const ApplicationList = ({ applications, onUpdateStatus, actionLoading, getStatusBadge }) => {
    ...
    return (
        <div className="space-y-4">
            {applications.map((application) => {
                ...
                return (
                    <Card key={appId} className="hover:shadow-md transition-shadow border-border/60">
                            ...
                            <div className="flex items-start justify-between">
                                <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <div>
                                            <h3 className="font-semibold text-foreground">{applicant.name || "Unknown Applicant"}</h3>
